...
export default function CustomeSelect({name, data, value, onChange, style}) {
  return (
    <div className={`${style} mb-4`}>
       <label htmlFor="custome-select" className="block mb-2 text-sm capitalize font-medium text-gray-900 dark:text-white">{name}</label>
        <select value={value} onChange={onChange} required id="custome-select" className="bg-gray-50 text-gray-900 text-sm rounded-lg 
